...
class Nano2(View):
    def get(self, request, hostname):
        resolutions = ['2160x1278', '1366x673', '3840x2400', '3840x2160', '2880x1800', '2560x1600', '2560x1440',
                       '1920x1440', '1856x1392', '1792x1344', '1920x1200', '1920x1080', '1600x1200', '1680x1050',
                       '1400x1050', '1280x1024', '1440x900', '1280x960', '1360x768', '1280x800', '1152x864',
                       '1280x768', '1280x720', '1024x768', '800x600', '640x480']
        functionalities = ['custom_cmd', 'reboot', 'screenshot', 'show_dashboard', 'show_video',
                           'change_resolution', 'upgrade']
        nano = NanoIoT.objects.all().get(hostname=hostname)
        where_dict = {}
        for place in NanoIoT.where_list:
            where_dict[place] = list(NanoIoT.objects.filter(where=place).values_list('hostname', flat=True))
